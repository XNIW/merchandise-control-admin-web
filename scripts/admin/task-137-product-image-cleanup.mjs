...
export function isCanonicalProductImageObjectPath(value) {
  return typeof value === "string" && canonicalObjectPathPattern.test(value);
}
...
export function isEligibleProductImageOrphan(
  ...
  return (
    isCanonicalProductImageObjectPath(object?.path) &&
    ...
    for (let offset = 0; ; offset += storagePageSize) {
      ...
      for (const item of page) {
        ...
        if (item.metadata && typeof item.metadata === "object") {
          objects.push({ createdAt: item.created_at, path: objectPath });
...
async function main() {
  ...
  const [candidatesResult, lifecyclePaths, storageObjects] = await Promise.all([
    supabase
      .from("inventory_product_image_versions")
      .select(
        "id,shop_id,product_id,status,created_at,expires_at,superseded_at,removed_at,main_path,thumb_path,requested_by_profile_id,actor_kind,cleanup_status",
      )
  ...
  for (const row of candidates) {
    if (row.status === "pending") {
      const failResult = await supabase.rpc("product_image_fail_version", {
        p_actor_kind: row.actor_kind,
        p_actor_profile_id: row.requested_by_profile_id,
        p_error_code: "cleanup_expired_pending",
    ...
    const removeResult = await bucket.remove([row.main_path, row.thumb_path]);
    const success = !removeResult.error;
  ...
  if (orphanCandidates.length > 0) {
    const orphanRemoveResult = await bucket.remove(
      orphanCandidates.map((object) => object.path),
    );
    if (orphanRemoveResult.error) {
      failed += orphanCandidates.length;
    } else {
      completed += orphanCandidates.length;
    }
  }
