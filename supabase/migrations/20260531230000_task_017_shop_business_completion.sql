...
-- Scope: additive member lifecycle RPCs for Shop Admin. No new tables, no direct
-- mutative grants, no hard deletes, and no service-role runtime dependency.
...
begin
  ...
  if p_shop_member_id is null then
    return app_private.shop_admin_action_result(false, 'validation_failed', p_shop_id);
  end if;
