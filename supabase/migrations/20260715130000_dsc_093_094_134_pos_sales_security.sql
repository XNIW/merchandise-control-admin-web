-- TASK-088 / DSC-093, DSC-094, DSC-134
-- Make POS sales authoritative for catalog prices/discount caps and bind every
-- refund/void line to one original line. The RPC owns the complete financial
-- write set so any validation or persistence failure rolls the whole batch back.
...
alter table public.staff_accounts
  drop constraint if exists staff_accounts_max_discount_percent_check,
  add constraint staff_accounts_max_discount_percent_check check (
    max_discount_percent between 0 and 100
  );

-- Preserve the reconciled POS Admin behavior with a real bounded staff cap.
-- Newly-created staff remain fail-closed at the column default until configured.
update public.staff_accounts
set max_discount_percent = 20
where role_key = 'pos_admin'
...
alter table public.staff_role_permissions
  ...
  add constraint staff_role_permissions_role_key_check check (
    role_key in ('cashier', 'manager', 'viewer', 'pos_admin')
  ),
...
-- Backward-safe defaults for permissions that were previously enforced only by
-- the TypeScript matrix. Existing disabled rows win; over-limit is never seeded.
...
cross join (
  values
    ...
    ('pos_admin', 'pos.sell'),
    ('pos_admin', 'pos.pay'),
    ('pos_admin', 'pos.refund'),
    ('pos_admin', 'pos.void'),
    ('pos_admin', 'pos.discount')
) permission(role_key, permission_key)
on conflict (shop_id, role_key, permission_key) do nothing;

alter table public.pos_sale_lines
  add column if not exists original_pos_sale_line_id uuid
    references public.pos_sale_lines(pos_sale_line_id) on delete restrict;

create index if not exists pos_sale_lines_original_line_idx
  on public.pos_sale_lines(original_pos_sale_line_id)
  where original_pos_sale_line_id is not null;
