-- TASK-114 post-DONE regression:
-- Extend the owner-scoped sync_events contract to history sessions so normal
-- foreground/reconnect sync can stay event/checkpoint incremental.

alter table public.sync_events
  drop constraint if exists sync_events_domain_mvp_check;

alter table public.sync_events
  add constraint sync_events_domain_task114_check
  check (domain in ('catalog', 'prices', 'history'));

alter table public.sync_events
  drop constraint if exists sync_events_event_type_mvp_check;

alter table public.sync_events
  add constraint sync_events_event_type_task114_check
  check (
    event_type in (
      'catalog_changed',
      'prices_changed',
