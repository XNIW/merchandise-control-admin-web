...
test("TASK-054 task and Master Plan tracking are aligned for DONE reconciliation", () => {
  ...
  assert.match(masterPlan, /Stato globale attuale: `(IDLE|REVIEW|EXECUTION|REVIEW_WITH_BLOCKERS|REVIEW_WITH_EXTERNAL_BLOCKERS|TASK_ACTIVE)`/);
  ...
  assert.match(
    masterPlan,
    /Task attivo: `(NESSUNO|Nessuno)`|Task attivo: `TASK-057 - Shop Catalog Workspace: prodotti, categorie, fornitori e import Excel intelligente`|Task attivo: `TASK-058 - Cloudflare\/OpenNext Staging Hardening and Deployment Governance`|Task attivo: `TASK-081 - Win7POS Sales Sync, Daily\/Monthly Revenue, Stock Sync and Shop Admin POS Revenue`|Task attivo: `TASK-065 - Fix Master Console Google OAuth redirect`|Task attivo: `TASK-075 - Admin Web performance audit e Products navigation latency fix`|Task attivo: `TASK-137 - Product Catalog Images cross-platform`|Task attivo: `TASK-138 - Product Images Runtime Completion, UX e Live Parity`/,
  );
  assertContains(masterPlan, "Stato TASK-055: `DONE_RECONCILED`");
  assertContains(masterPlan, "Stato TASK-056: `DONE_RECONCILED`");
  assert.match(
    masterPlan,
    /Ultimo task chiuso: `TASK-056 - Master Console shop detail editing and row navigation shortcut`|Ultimo task chiuso: `TASK-057 - Shop Catalog Workspace: prodotti, categorie, fornitori e import Excel intelligente`|Ultimo task chiuso: `TASK-059 - Post-merge Supabase Staging Readiness`|Ultimo task chiuso: `TASK-060 - Supplier Excel Android-style preview\/import`|Ultimo task chiuso: `TASK-061 - Android database export compatibility for Admin Web database transfer`|Ultimo task chiuso: `TASK-064 - Master Console Auth\/Profile Parity e ricerca utenti Android\/iOS`|Ultimo task chiuso: `TASK-067 - Master Console lifecycle management, admin assignment, and safe cleanup`|Ultimo task chiuso: `TASK-071 - Final closure and fix-all for TASK-068 \/ TASK-069 \/ TASK-070`|Ultimo task chiuso: `TASK-072 - Cross-platform catalog sync and History Entry write path for Admin Web, Android and iOS`|Ultimo task chiuso: `TASK-073 - Account identity display globale`|Ultimo task chiuso: `TASK-074 - Devices UX Polish \/ Owner-Friendly Device Registry`|Ultimo task chiuso: `TASK-077B - Performance architecture fix: Products \+ Master Console lightweight read models`|Ultimo task chiuso: `TASK-078C - Product Detail visual polish and History Entries month-grouped UX`|Ultimo task chiuso: `TASK-079 - History Entry and Catalog Pagination Unified Completion`|Ultimo task chiuso: `TASK-081 - Win7POS Sales Sync, Revenue, Stock Sync, Shop Admin Dashboard and UX alignment`/,
  );
  assert.doesNotMatch(
    masterPlan,
