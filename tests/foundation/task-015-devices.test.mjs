...
test("TASK-015 devices use a shop-scoped authorization registry with audited mutations", () => {
  ...
  assert.match(migration, /alter table public\.shop_devices enable row level security/);
  assert.match(migration, /status in \('pending', 'active', 'revoked', 'suspicious'\)/);
  ...
  assert.doesNotMatch(`${readModel}\n${mutations}`, /device_secret|device_token|credential_hash/i);
});
...
test("TASK-015 devices page exposes guarded register, rename, revoke and reactivate controls", () => {
  ...
  assert.match(page, /getShopSectionForRequest\(\s*"devices"/);
  ...
  assert.match(sectionData, /Server registry/);
