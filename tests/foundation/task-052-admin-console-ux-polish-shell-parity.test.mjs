...
test("TASK-052 protected console shells disable auth-breaking prefetch", () => {
  ...
  assert.match(
    shopShell,
    /<form[\s\S]*principalKind === "pos_staff_manager"[\s\S]*"\/shop\/staff-logout"[\s\S]*"\/auth\/logout\?next=\/shop"[\s\S]*method="post"[\s\S]*>\s*<button[\s\S]*type="submit"[\s\S]*>\s*\{logoutLabel\}\s*<\/button>/,
  );
  assert.match(platformShell, /<form[\s\S]*action="\/auth\/logout\?next=\/platform"[\s\S]*method="post"[\s\S]*>\s*<button[\s\S]*type="submit"[\s\S]*>\s*\{dictionary\.common\.logout\}\s*<\/button>/);
