...
export async function GET(request: Request) {
  ...
  if (!uuidPattern.test(shopId) || !uuidPattern.test(saleId)) {
  ...
  const result = await callWeChatUserRpc({
    ...
    params: { p_pos_sale_id: saleId, p_shop_id: shopId },
    rpc: "wechat_sale_detail_v2",
  });
  return NextResponse.json(result.ok ? { detail: result.data, ok: true } : result, {
