...
import { exchangeWeChatCode } from "@/server/auth/wechat-exchange";

export const dynamic = "force-dynamic";
export const runtime = "nodejs";

const base64UrlPattern = /^[A-Za-z0-9_-]{43,128}$/;
const codePattern = /^[A-Za-z0-9_-]{1,512}$/;
const correlationPattern = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
const deviceIdPattern = correlationPattern;
...
export async function POST(request: Request) {
  ...
  if (
    ...
    !base64UrlPattern.test(body.state) ||
    typeof body.nonce !== "string" ||
    !base64UrlPattern.test(body.nonce) ||
    typeof body.deviceId !== "string" ||
    !deviceIdPattern.test(body.deviceId) ||
    typeof body.correlationId !== "string" ||
    !correlationPattern.test(body.correlationId)
  ) {
    return NextResponse.json(
      { code: "code_invalid", ok: false },
      { status: 400 },
    );
  }
  ...
  const outcome = await exchangeWeChatCode(
    {
  ...
  return NextResponse.json(outcome.ok ? outcome.session : outcome, {
